...
const [cart, setCart] = useState([]); const [isSubmitting, setIsSubmitting] = useState(false); const [submitted, setSubmitted] = useState(false);

const handleChange = (e) => { setFormData({ ...formData, [e.target.name]: e.target.value }); };

const addToCart = (title) => { setCart([...cart, title]); };

const handleSubmit = async (e) => { e.preventDefault(); setIsSubmitting(true);

const fullOrder = cart.length > 0 ? cart.join(", ") : formData.item;
const fullQuantity = formData.quantity || cart.length;
...
{/* Hero Section */}
  ...
  <section id="menu" className="py-12 bg-white text-center">
    ...
    <div className="grid md:grid-cols-3 gap-6 px-8">
      {[{ src: `${process.env.PUBLIC_URL}/bread.jpg`, title: "Breads", desc: "Crusty sourdough, soft rolls, and more." },
        ...
        { src: `${process.env.PUBLIC_URL}/pastry1.jpg`, title: "Pastries", desc: "Swirls, tarts, puffs and cookies with love." }].map((item, i) => (
        <div key={i} className="bg-yellow-50 shadow-md rounded-lg p-4" data-aos="zoom-in">
          <img src={item.src} alt={item.title} className="w-full h-84 object-cover rounded" />
          <h3 className="mt-4 text-xl font-heading">{item.title}</h3>
          <p className="mt-2 text-sm text-gray-600 font-body">{item.desc}</p>
          <button onClick={() => addToCart(item.title)} className="mt-3 px-4 py-2 bg-yellow-500 text-white rounded hover:bg-yellow-600 transition">
            Add to Cart
          </button>
        </div>
      ))}
  ...
  {cart.length > 0 && (
    <section className="py-6 bg-pink-50 text-center">
      <h3 className="text-xl font-heading mb-2">🛒 Your Cart</h3>
      <ul className="inline-block text-left bg-white p-4 rounded shadow">
        {cart.map((item, idx) => (
          <li key={idx} className="text-gray-700 font-body">• {item}</li>
        ))}
      </ul>
    </section>
  )}
