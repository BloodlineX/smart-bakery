import React, { useEffect, useState } from "react"; import AOS from "aos"; import "aos/dist/aos.css";

function App() { useEffect(() => { AOS.init({ duration: 1000 }); }, []);

const [formData, setFormData] = useState({ name: "", phone: "", item: "", quantity: "", message: "", });

const [isSubmitting, setIsSubmitting] = useState(false); const [submitted, setSubmitted] = useState(false); const [cart, setCart] = useState([]);
...
<section className="relative text-center py-10 bg-pink-100">
  ...
  <section id="menu" className="py-12 bg-white text-center">
    ...
    <div className="grid md:grid-cols-3 gap-6 px-8">
      {[
        { src: `${process.env.PUBLIC_URL}/cake1.jpg`, title: "Cakes", items: ["Chocolate", "Vanilla", "Red Velvet", "Butterscotch"] },
        { src: `${process.env.PUBLIC_URL}/pastry1.jpg`, title: "Pastries", items: ["Black Forest", "Pineapple", "Choco Chip", "Strawberry"] },
        { src: `${process.env.PUBLIC_URL}/bread.jpg`, title: "Breads", items: ["Garlic Bread", "White Bread", "Multigrain", "Focaccia"] },
        { src: `${process.env.PUBLIC_URL}/cookie.jpg`, title: "Cookies", items: ["Choco Chip", "Butter", "Oats", "Almond"] },
        { src: `${process.env.PUBLIC_URL}/muffin.jpg`, title: "Muffins", items: ["Chocolate", "Blueberry", "Banana", "Vanilla"] },
        { src: `${process.env.PUBLIC_URL}/Cupcake.jpg`, title: "Cupcakes", items: ["Red Velvet", "Chocolate", "Vanilla", "Lemon"] },
      ].map((item, i) => (
          ...
          <ul className="mt-2 text-sm text-gray-600 font-body text-left list-disc list-inside">
            {item.items.map((flavour, j) => (
              <li key={j} className="flex justify-between">
                <span>{flavour}</span>
                <button className="text-xs text-yellow-700 underline ml-2" onClick={() => addToCart(`${item.title} - ${flavour}`)}>Add to Cart</button>
              </li>
            ))}
          </ul>
        </div>
  ...
  <section className="py-10 bg-yellow-50 text-center">
    <button
      ...
      className="text-2xl font-heading text-yellow-900 mb-4 hover:underline focus:outline-none"
    >
      🛒 Your Cart
    </button>
    {cart.length > 0 ? (
